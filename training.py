...
def main(args):
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        ...
        model = modeling[0](embed_dim=128, num_layer=1, device=device).to(device).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=LR)
        best_mse = 1000
        best_ci = 0
        best_epoch = -1
        # model_file_name = 'model' + model_st + '_' + dataset +  '.model'
        result_file_name = 'result' + model_st + '_' + dataset + '-8.csv'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_file", type=str,
                        default="C:/Users/64478/Desktop/xfy666888/Graph&BiLSTM-ZZ/pretrained_model/davisGCN-8",
