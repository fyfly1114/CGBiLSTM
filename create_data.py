...
# from DeepDTA data
#从folds文件中分别生成drus,prot,affinity.csv文件
all_prots = []
datasets = ['kiba', 'davis']
for dataset in datasets:
    ...
    opts = ['train', 'test']
    for opt in opts:
        rows, cols = np.where(np.isnan(affinity) == False)#返回affinity中不是空值处的行和列。即返回每个值对应的横纵坐标
        #np.isnan()是判断是否是空值。np.where(condition) 当where内只有一个参数时，那个参数表示条件，当条件成立时，where返回的是每个符合condition条件元素的坐标,返回的是以元组的形式
        if opt == 'train':
